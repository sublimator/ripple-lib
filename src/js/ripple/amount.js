...
// be sure that _is_native is set property BEFORE calling _set_value
...
Amount.prototype.is_comparable = function(v) {
  return this._is_native === v._is_native;
};
...
Amount.prototype.parse_human = function(j, opts) {
  ...
  if (words.length === 1 && isNum(words[0])) {
    value = words[0];
    currency = 'XRP';
...
Amount.prototype.parse_quality = function(quality, counterCurrency, counterIssuer, opts)
{
  ...
  var value         = mantissa.times((new BigNumber(10)).toPower(offset));
...
Amount.prototype.to_text = function(allow_nan) {
  ...
  var value = (new BigNumber(this._value)).times(ten.toPower(-offset));
  var sign = value.isNegative() ? '-' : '';
  var mantissa = value.absoluteValue();
